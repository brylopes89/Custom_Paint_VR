...
namespace IndiePixel.VR
{
    ...
    public class IP_VR_RadialMenu : MonoBehaviour 
    {
        ...
        public SteamVR_Action_Boolean controller_Action = SteamVR_Input.GetAction<SteamVR_Action_Boolean>("InteractUI");
    	...
    	void Start () 
        {
            ...
            if(controller)
            {
               /* controller_Action = SteamVR_Controller.Input((int)controller.controllerIndex);
...
//            HandleDebugText("Clicked Pad");
        ...
        void UpdateMenu()
        {
            if(isTouching)
            {
                ...
                //currentAxis = controller_Action.GetAxis();
